...
export function registerNotes(program: Command) {
  ...
  notesCmd
    ...
    .action(async (opts: { id: string }) => {
      await run(program, () => domain.notes.get(getUser(program), opts.id));
    });
  ...
  notesCmd
    ...
    .action(
      async (opts: {
        ...
        await run(program, () =>
          domain.notes.update(getUser(program), opts.id, {
            title: opts.title,
            content: opts.content,
            pinned:
  ...
  notesCmd
    ...
    .requiredOption('--id <id>', 'Note ID')
    .action(async (opts: { id: string }) => {
      await run(program, () => domain.notes.remove(getUser(program), opts.id));
    });
  ...
  notesCmd
    ...
    .action(async (opts: { id: string; tag: string }) => {
      await run(program, () =>
        domain.notes.addTag(getUser(program), opts.id, opts.tag)
      );
    });
  ...
  notesCmd
    ...
    .action(async (opts: { id: string; tag: string }) => {
      await run(program, () =>
        domain.notes.removeTag(getUser(program), opts.id, opts.tag)
      );
    });
}
